...
- Exception: exploratory spikes (e.g. verifying a toolchain capability such as cross-compiling to iOS) may skip test-first with explicit user agreement; discard or rewrite the spike as a proper implementation afterward
...
Scope is optional; use the package name when the change targets a specific package (e.g., `logic`, `ffi`). Omit for project-wide changes.
